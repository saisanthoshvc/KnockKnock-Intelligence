...
# We don't know what header/scheme your extractor validates. We try several.
AUTH_CANDIDATES = [
...
def api_post_with_fallbacks(files, data, want_sync: bool) -> Tuple[Optional[requests.Response], dict, str]:
    ...
    for path in paths:
        ...
        for build in AUTH_CANDIDATES:
            hdrs = build(API_KEY)
            try:
                # Let requests set multipart boundary; don't force Content-Type
                ...
                if r.status_code == 200:
                    return r, hdrs, path
                # Common auth errors: keep trying next header
                if r.status_code in (401, 403):
                    last_error = (r.status_code, r.text[:500])
                    continue
                # FastAPI validation (422) or custom 400: keep trying only if text hints pattern mismatch
                if r.status_code in (400, 422) and "pattern" in (r.text or "").lower():
                    last_error = (r.status_code, r.text[:500])
                    continue
                # Other statuses—still return so caller can surface message
                return r, hdrs, path
